...
sudo apt-get install -y unzip git python3-pip build-essential wget python3-dev python3-venv python3-wheel libxslt-dev libzip-dev libldap2-dev libsasl2-dev python3-setuptools node-less fail2ban
...
[options]
   ...
   db_user = odoo
   db_password = False
   addons_path = /opt/odoo_enterprise,/opt/odoo/addons,/opt/caliva_prod/odoo
...
#Keep dump file in the same folder with name as dump.sql
unzip /home/ubuntu/caliva_2021-12-31.zip
...
cd filestore
sudo cp -r /home/ubuntu/filestore .
sudo mv filestore calivaengineering-odoo-caliva-98549 
sudo chown -R odoo: calivaengineering-odoo-caliva-98549/
#adding custom addons
cd
#replacing odoo source code
sudo tar xvfz odoo11.tar.gz 
sudo mv odoo11 /opt/
cd /opt
sudo rm -rf odoo
sudo mv odoo11 odoo
...
cd
#replacing odoo-enterprise
unzip odoo11_enterprise.zip
cd odoo11_enterprise
sudo mv odoo_enterprise /opt/
sudo chown -R odoo: /opt/odoo_enterprise/

cd
#custom addons
unzip caliva_prod.zip

sudo cp -r caliva_prod /opt/caliva_prod
cd /opt
sudo chown -R odoo: caliva_prod/
cd /opt/caliva_prod/odoo 
sudo pip3 uninstall -y pillow
sudo apt-get install -y libjpeg-dev zlib1g-dev
sudo pip3 install -y pillow
sudo pip3 install -r /opt/caliva_prod/odoo/requirements.txt
sudo su - odoo -c "pip3 install -r /opt/caliva_prod/odoo/requirements.txt"

sudo apt-get install -y libqpdf-dev
python3 -m pip install --upgrade pip
python3 -m pip install pikepdf
sudo python3 -m pip install pikepdf
